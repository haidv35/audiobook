@if($product->regular_price == 0 && $product->discount_price == 0)
    <a href="{{ route('free',['id'=>$product->id]) }}" class="btn btn-outline-success @if($view === 'all-product') ?  btn-block : '' @else btn-sm  @endif float-right {{Auth::user() ? '' : 'login-before_order' }}">Xem nội dung sản phẩm <i
        class="fa fa-eye"></i></a>
@else
    @php
        $checkOrdered = 0;
